#!/usr/bin/env python3
"""조성환 파주시장 후보 뉴스 수집기 — 네이버 뉴스 API + 댓글 크롤링 + 감성분석"""
...
# ──────────────────── 검색 키워드 ────────────────────
PRIMARY_KEYWORDS = [
    "조성환", "조성환 파주시장", "조성환 후보", "조성환 공약",
    "조성환 출마", "조성환 민주당", "조성환 파주",
]
COMPETITOR_KEYWORDS = [
    "파주시장 선거 2026", "파주시장 후보",
    "파주시장 여론조사", "파주시장 경선",
    "파주시장 민주당 경선",
    "이용욱 파주", "고준호 파주", "안명규 파주",
]
ISSUE_KEYWORDS = [
    "파주 교통", "파주 접경지역", "파주 발전",
    "파주 GTX", "파주 개발", "파주 일자리",
...
def analyze_sentiment(text, target_candidate="조성환"):
    """감성분석 — 가중치 키워드 + 문맥 근접도"""
    pos_score = 0
    neg_score = 0
...
def collect_all_news(target_date):
    ...
    for kw in ALL_KEYWORDS:
        ...
        for item in items:
            ...
            # 조성환 또는 파주시장 관련 기사만
            is_relevant = any(name in full_text for name in CANDIDATE_NAMES) or "파주시장" in full_text or "파주" in full_text
            if not is_relevant:
                continue
            ...
            all_articles[link] = {
                ...
                "is_joseonghwan": "조성환" in full_text,
                "comments": {
...
def compute_daily_stats(articles, target_date):
    """일일 통계 계산"""
    js_articles = [a for a in articles if a.get("is_joseonghwan")]
    total = len(articles)
    js_total = len(js_articles)

    # 감성 비율
    sentiments = {"긍정": 0, "부정": 0, "중립": 0}
    for a in js_articles:
        sentiments[a.get("sentiment", "중립")] += 1

    total_comments = sum(a["comments"]["count"] for a in js_articles)
    pos_comments = sum(a["comments"]["positive"] for a in js_articles)
    neg_comments = sum(a["comments"]["negative"] for a in js_articles)
    neu_comments = sum(a["comments"]["neutral"] for a in js_articles)
    ...
    # 주요 키워드 추출
    keyword_freq = {}
    issue_words = ["교통", "접경지역", "발전", "GTX", "개발", "일자리", "경제",
                   "안전", "환경", "교육", "의료", "문화", "안보", "통일", "평화",
    ...
    pos_pct = round(sentiments["긍정"] / js_total * 100) if js_total > 0 else 0
    neg_pct = round(sentiments["부정"] / js_total * 100) if js_total > 0 else 0
    neu_pct = 100 - pos_pct - neg_pct

    return {
        "date": target_date,
        "total_articles": total,
        "joseonghwan_articles": js_total,
        "sentiment": {
...
def main():
    parser = argparse.ArgumentParser(description="조성환 뉴스 수집기")
    parser.add_argument("--date", default=datetime.now().strftime('%Y-%m-%d'))
    args = parser.parse_args()

    load_env()
    print(f"[START] 조성환 뉴스 수집 시작 — {args.date}")

    articles = collect_all_news(args.date)
    print(f"[INFO] 수집된 기사: {len(articles)}건")

    stats = compute_daily_stats(articles, args.date)
    print(f"[INFO] 조성환 관련: {stats['joseonghwan_articles']}건 "
