...
ACCOUNTS = {
    "youtube": {
        "channel_id": "",  # @pajusd
        "handle": "pajusd",
        "search_keywords": ["조성환", "조성환 파주시장"],
    },
    "facebook": {
        "page_url": "pajusindong",
        "search_keywords": ["조성환"],
    },
    "instagram": {
        "username": "pajusindong",
    },
    "blog": {
        "url": "pajusindong",  # 네이버 블로그
    },
}
...
def collect_youtube():
    ...
    if YOUTUBE_API_KEY:
        for keyword in ["조성환 파주시장", "조성환 파주"]:
            try:
    ...
    if naver_id and naver_secret:
        for search_type in ["cafearticle", "blog"]:
            try:
                ...
                params = {"query": "조성환 파주시장", "display": 20, "sort": "date"}
                resp = requests.get(url, headers=headers, params=params, timeout=10)
                data = resp.json()
...
def collect_naver_reactions():
    ...
    params = {"query": "조성환 파주", "display": 5, "sort": "date"}
    try:
        resp = requests.get(url, headers=headers, params=params, timeout=10)
...
def main():
    parser = argparse.ArgumentParser(description="조성환 SNS 수집기")
    parser.add_argument("--date", default=datetime.now().strftime('%Y-%m-%d'))
    args = parser.parse_args()

    print(f"[START] 조성환 SNS 수집 시작 — {args.date}")
