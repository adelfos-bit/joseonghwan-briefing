...
def compute_trends(today_stats, prev_data):
    ...
    today_count = today_stats.get("joseonghwan_articles", 0)
    prev_count = prev_stats.get("joseonghwan_articles", 0)
...
def build_strategy_prompt(news_data, social_data, trends, target_date):
    ...
    neg_articles = [a for a in articles if a.get("sentiment") == "부정" and a.get("is_joseonghwan")]
    neg_text = "\n".join([f"  - [{a['sentiment']}] {a['title']}" for a in neg_articles[:5]]) or "없음"

    # 경쟁자별 부정 기사 (공격 포인트)
    comp_weaknesses = ""
    for comp_name in ["이용욱", "고준호", "안명규"]:
    ...
    prompt = f"""당신은 조성환 파주시장 후보 캠프의 수석 전략참모입니다.
당신의 유일한 목표는 조성환의 당선입니다. 아래 오늘({target_date}, D-{d_day}) 수집 데이터를 분석해서 조성환 캠프에 전략 지시를 내려주세요.

## 배경
- 조성환: 더불어민주당 경기도의원, 파주시장 출마
- 민주당 경선 경쟁자: 이용욱 (경기도의원, 공식 출마선언)
- 국민의힘: 고준호 (공식 출마선언), 안명규 (출판기념회 의지 표명), 박용호 (당협위원장)
- 기타: 이재홍 (출마 확정), 한길룡 (유보적)
- 핵심 이슈: 교통 인프라, 접경지역 활성화, 파주 발전

## 오늘의 데이터

### 조성환 현황
- 기사 {stats.get('joseonghwan_articles', 0)}건 / 전체 {stats.get('total_articles', 0)}건
...
아래 형식으로 **조성환 캠프 전략 지시서**를 작성하세요. 텔레그램 메시지로 전송되므로 간결하게 핵심만 작성하세요.

━━━ 핵심 진단 ━━━
(2-3문장. 오늘 조성환에게 가장 중요한 상황 판단. 수치 기반.)

🎯 핵심 미션
"(한 문장. 오늘 조성환이 집중해야 할 것)"
...
- 모든 제안은 "조성환이 오늘/이번주 실행 가능한 것"이어야 함
- 파주 지역 특성(접경지역, 교통, 개발) 고려
- 800자 이내로 작성"""
...
def generate_fallback_strategy(news_data, trends):
    ...
    js_count = stats.get("joseonghwan_articles", 0)
    total = stats.get("total_articles", 0)
    exposure = stats.get("candidate_exposure", {})
    top_comp = max([(k, v) for k, v in exposure.items() if k != "조성환"], key=lambda x: x[1], default=("이용욱", 0))
    ...
    if neg_pct > 40:
        ...
        strategy["core_diagnosis"] = (
            f"📊 경선/토론 국면. 기사 {js_count}건 중 경선·토론 키워드 집중. "
            f"당내 경쟁 속 차별화 메시지가 핵심. {top_comp[0]} {top_comp[1]}건 노출 중."
        )
        strategy["top_priority"] = "경선 토론 차별화 — 파주 발전 비전 1~2개에 집중"
    elif "공약" in keywords or "교통" in keywords or "접경지역" in keywords:
        strategy["core_diagnosis"] = (
            f"📋 정책 경쟁 국면. 공약/교통/접경지역 관련 보도 집중. "
        ...
        strategy["core_diagnosis"] = (
            ...
            f"조성환 {js_count}건 vs {top_comp[0]} {top_comp[1]}건. "
    ...
    exposure = stats.get("candidate_exposure", {})
    top_competitor = max(
        [(k, v) for k, v in exposure.items() if k != "조성환"],
        key=lambda x: x[1], default=("이용욱", 0)
    )
...
def main():
    parser = argparse.ArgumentParser(description="조성환 전략 분석")
